...
## Current Commands
Right now the only commands that are available are CSTRING, SET, GET, DEL, STATS, PURGE. These commands do work but there's no validation on the data itself. It's literally bare bones.

## Querying The Server
Currently there's a hardcoded connection string (will soon move to a config file) to get started, if connecting from telnet, after connecting use `CSTRING <connection string>` to authenticate with the server. The current method of setting data is `0 set {key} {value}` where `0` is the DB store, `{set}` is the command, `{key}` for what key you want to store it in, and of course the `{value}`. You can't have duplicate keys in the same DB as `set` will just overwrite the key. `0 get key` will retrieve the value along with `0 del key` for deleting it.
...
## Future Plans
Not much, just get it stable, add a few more commands, maybe throw in actual JSON support, ~~nnection strings~~, TLS, etc.
